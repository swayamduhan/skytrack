...
export async function POST(req : NextRequest){
    ...
    if(type === "NEW_USER"){
        ...
        text = `Hi ${username},\nYour flight from ${body.from} to ${body.to} on ${body.date} at ${body.time} has dropped below ${body.threshold} ${body.currency}! Current price is ${body.price} Consider booking your flight ✈️\n\nRegards,\nSkytrack`
    } else if (type === "FLIGHT_DISCOUNT"){
        subject = "Flights at discount!"
        text = `Hi ${username},\nYour starred flight from ${body.from} to ${body.to} on ${body.date} at ${body.time} is available at a discounted price ${body.price} ${body.currency}, slightly higher than your threshold ${body.threshold}.\n\nRegards,\nSkytrack`
